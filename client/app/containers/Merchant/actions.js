...
export const addMerchant = (isBack = false) => {
    return async (dispatch, getState) => {
        try {
            const internationalPhoneRegex = /^(?:\+?[1-9]\d{7,14}|0\d{9,14})$/;
            ...
            const normalizedPhone = (merchant.phoneNumber || "").replace(
                /[\s().-]/g,
                "",
            );
            ...
            if (!internationalPhoneRegex.test(normalizedPhone)) {
                return dispatch({
                    type: SET_MERCHANT_FORM_ERRORS,
                    payload: {
                        phoneNumber: [
                            "Phone number format is invalid. Use a valid local or international format.",
                        ],
                    },
                });
            }
...
export const disableMerchant = (merchant, value, search, page) => {
    return async (dispatch, getState) => {
        try {
            console.log("[Merchant] Updating active status", {
                merchantId: merchant?._id,
                nextIsActive: value,
            });
            const response = await axios.put(`${API_URL}/merchant/${merchant._id}/active`, {
                merchant: {
                    isActive: value,
                },
            });
            console.log("[Merchant] Active status updated", response?.data);

            dispatch({
                type: UPDATE_MERCHANT_ACTIVE_STATUS,
                payload: { id: merchant._id, isActive: value },
            });
            ...
            dispatch(
                success({
                    title:
                        ...
                        `Merchant has been ${value ? "enabled" : "disabled"}.`,
                    position: "tr",
                    autoDismiss: 1,
                }),
            );
            ...
            console.error("[Merchant] Failed to update active status", error);
            handleError(error, dispatch);
        }
    };
};
